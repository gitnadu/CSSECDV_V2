...
export const EnrollmentRepository = {
  ...
  /**
   * Find enrollment by ID with full section and course details
   */
  ...
  /**
   * Find enrollments by student ID with full section and course details
   * Returns enriched data including course_code, course_name, section_name, professor_name
   */
  async findByStudentIdWithDetails(studentId) {
    const result = await query(
      `
        SELECT 
          e.id,
  ...
  async findByProfessorIdWithDetails(professorId) {
    const result = await query(
      `
        SELECT 
          e.id,
        ...
        JOIN courses c ON s.course_id = c.id
        LEFT JOIN users u ON s.professor_id = u.id
        LEFT JOIN users su ON e.student_id = su.id
