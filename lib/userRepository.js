...
export const UserRepository = {
  ...
  async create(userData) {
    ...
    const result = await query(
      `
      INSERT INTO users (username, password_hash, role, first_name, last_name, email, password_changed_at)
    ...
    try {
      ...
      console.error('[UserRepository] Error storing initial password history:', err);
   ...
   * @returns {boolean} - True if password was used before, false otherwise
   */
  async checkPasswordHistory(userId, newPassword, historyCount = 5) {
    try {
      ...
      console.error('[UserRepository] Error checking password history:', error);
      // In case of error, allow password change to avoid blocking users
      return false;
    }
  },
  ...
  async update(id, userData) {
    ...
    if (password !== undefined) {
      ...
      try {
        ...
        console.error('[UserRepository] Error storing password history:', err);
        // Continue with password update even if history storage fails
      }
    }
   ...
   * Returns the last login timestamp and whether it was successful
   */
  async getLastLoginInfo(userId) {
    const result = await query(
      `SELECT last_login_at, last_login_success, last_failed_login_at 
   ...
   * Stores the previous login info before updating
   */
   ...
   * Updates the user's last_failed_login_at and marks last_login_success as false
   */
